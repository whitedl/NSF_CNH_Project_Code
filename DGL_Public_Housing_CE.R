...
# Data Directory: C:\Users\admin\Box Sync\Default Sync Folder\Projects\NSF_CNH\HUD_Analysis\Section8\DGL_AHD.csv

DGL_AHD <- read.csv("C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/Tables_In/DGL_HUD.csv", stringsAsFactors=FALSE)
DGL_AHD_MAX <- DGL_AHD %>% group_by(GIS_JOIN) %>% filter(YEAR == max(YEAR)) 
DGL_AHD_Edit <- DGL_AHD_MAX  %>% select(Tract_Num = GIS_JOIN,YEAR,Sec_8_Reported = NUMBER_REPORTED)
...
DGL_Census_Poly <- sf:::st_read(dsn = "C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/ShapeFiles_In", layer = "DGL_Census_Tract")
...
#DGL_out_shape  = tempfile(pattern = "DGL_AHD_", tmpdir = "C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/CensusTract_2000", fileext = ".shp")
# Exports to a Shapefile
sf:::st_write(DGL_Census_AHD, "C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/ShapeFiles_Out/DGL_Census_AHD.shp", delete_layer=TRUE)
#Plot to Check 
...
DGL_CE_poly <- st_read("C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/ShapeFiles_In/DGL_CE_data.shp")
DGL_CE_poly <- st_transform(DGL_CE_poly, "+proj=longlat +ellps=GRS80 +datum=NAD83 +no_defs")
...
#DGL_AHD_Tax_spatial_join_FINAL_shp  = tempfile(pattern = "DGL_AHD_Tax_spatial_join_FINAL_", tmpdir = "C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/CensusTract_2000", fileext = ".shp")
# Exports to a Shapefile
sf::st_write(DGL_Census_AHD_FINAL, "C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/ShapeFiles_Out/DGL_AHD_Tax_spatial_join_FINAL.shp",  delete_layer=TRUE)

#Export data frame only
DGL_Census_AHD_FINAL_table <- dplyr::select(as.data.frame(DGL_Census_AHD_FINAL), -geometry, -row.id)
write.csv(DGL_Census_AHD_FINAL_table, "C:/Users/admin/Box Sync/Default Sync Folder/Projects/NSF_CNH/HUD_Analysis/FinalTables_Out/DGL_AHD_Tax_spatial_join_FINAL_table.csv" )
